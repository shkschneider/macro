...
type model struct {
	...
	err          error
	showPicker   bool
	activeDialog Dialog // Single active dialog (nil when closed)
}
	...
	case tea.KeyMsg:
		...
		if msg.String() == "ctrl+ " {
			if m.getCurrentFilePath() != "" {
				m.activeDialog = NewFileDialog(filepath.Dir(m.getCurrentFilePath()))
		...
		if msg.String() == "ctrl+b" {
			if len(m.buffers) > 0 {
				m.activeDialog = NewBufferDialog(m.buffers, m.currentBuffer)
				return m, m.activeDialog.Init()
			} else {
